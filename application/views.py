...
def login_view(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('dashboard' or 'admin_dashboard')
            else:
    ...
    return render(request, "user-dashboard/login.html", {"form": form, "msg": msg})
...
def dashboard(request):
    ...
    if not (hasattr(user, 'Hemis_admin') or hasattr(user, 'Moodle_admin') or hasattr(user, 'KeroControl_admin')):
        return redirect('login')  # Foydalanuvchi admin profili emas, login sahifasiga yo'naltiriladi

    # Agar admin profili bo'lsa, uni boshqarish panelini chiqaramiz
